...
export const mapRowToSuggestion = (row: any): Suggestion => {
  ...
  const resolvedOfficialResponse = extracted.officialResponse
    ? extracted.officialResponse
    : (row.admin_reply
      ? {
          authorName: '학생자치부장',
          department: '제53대 학생회',
...
export const addCommentToSupabase = async (
  ...
  try {
    ...
    const commentToAdd = {
      ...newComment,
      authorNickname: newComment.authorNickname.trim() || '익명의 삼진인',
      content: newComment.content.trim(),
    };

    // Deduplicate comment if already exists
    const updatedComments = deduplicateComments([...existing, commentToAdd]);
...
export const updateStatusInSupabase = async (
  ...
  // Variant 1: Update status + content with encoded official response
  try {
    const { data: current } = await supabase
      .from('suggestions')
      .select('*')
    ...
    if (current) {
      ...
      const { data, error } = await supabase
        .from('suggestions')
        .update({ status: mapStatusToDB(status), content: newContent, ...updatePayload })
        .eq('id', id)
        .select()
        .single();
  ...
  // Variant 2: Update status + admin_reply column
  try {
    const { data, error } = await supabase
      .from('suggestions')
