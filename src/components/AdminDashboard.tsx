            ...
            <>
              ...
              <div className={`p-4 rounded-2xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${isMaintenanceMode ? 'bg-amber-500 animate-ping' : 'bg-emerald-500'}`} />
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-extrabold text-sm">
                        {isMaintenanceMode ? '🚧 사이트 점검 모드 활성화 중' : '🟢 사이트 정상 운영 중'}
                      </span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
                        isMaintenanceMode ? 'bg-amber-200 text-amber-900' : 'bg-emerald-200 text-emerald-900'
                      }`}>
                        {isMaintenanceMode ? '일반 학생 접속 차단 (점검 화면 표시)' : '일반 학생 자유 접근 가능'}
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 mt-0.5">
                      {isMaintenanceMode 
                        ? '현재 일반 학생들이 접속하면 점검 안내 화면이 뜹니다. 학생회는 관리자 PIN으로 자유롭게 접근 가능합니다.' 
                        : '모든 학우가 건의사항을 작성하고 열람할 수 있습니다.'}
                    </p>
                  </div>
                </div>
                ...
                {onToggleMaintenanceMode && (
                  <button
                    ...
                    className={`text-xs font-bold px-4 py-2 rounded-xl transition-all shadow-xs shrink-0 self-end sm:self-auto ${
                      isMaintenanceMode
                        ? 'bg-emerald-600 hover:bg-emerald-700 text-white'
                        : 'bg-amber-600 hover:bg-amber-700 text-white'
                    }`}
                  >
                    {isMaintenanceMode ? '✨ 점검 모드 해제 (사이트 열기)' : '🛠️ 사이트 점검 모드로 전환 (닫기)'}
                  </button>
                )}
              </div>
                  ...
                  {suggestions.map((s) => (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-slate-500 truncate">{s.authorNickname || '익명의 삼진인'} • 공감 {s.upvotes}개</p>
                      </div>
