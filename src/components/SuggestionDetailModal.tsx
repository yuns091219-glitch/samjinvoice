...
export const SuggestionDetailModal: React.FC<SuggestionDetailModalProps> = ({
  ...
  // Preset choices for quick selection
  const AUTHOR_ROLE_PRESETS = [
    '학생회장',
    '학생자치부장',
    '학생인권부장',
  ];
  ...
  useEffect(() => {
    ...
    // Reset admin response state to current suggestion data
    setNewStatus(suggestion.status);
    setResponseDepartment('제53대 학생회');
    setResponseAuthor(suggestion.officialResponse?.authorName || '학생자치부장');
    setResponseContent(suggestion.officialResponse?.content || '');
        ...
        .then((data) => {
          if (data && !data.error) {
            ...
            if (data.officialResponse) {
              setResponseDepartment('제53대 학생회');
              setResponseAuthor(data.officialResponse.authorName || '학생자치부장');
              setResponseContent(data.officialResponse.content || '');
            }
          }
        })
        .catch(() => {});
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl max-w-3xl w-full max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/80">
          <div className="flex items-center space-x-2 flex-wrap gap-y-1">
            {(() => {
              const normCat = normalizeCategory(activeSuggestion.category);
              const CatIcon = CATEGORY_ICONS[normCat] || CATEGORY_ICONS['OTHER'];
              return (
                <span className="inline-flex items-center gap-1.5 text-xs font-semibold bg-[#F4F1EA] text-[#4A443F] px-2.5 sm:px-3 py-1 rounded-xl border border-[#E6E2D3]">
            ...
            {isMyPost && (
              <span className="text-xs font-semibold bg-emerald-50 text-[#3e5341] px-2.5 py-1 rounded-xl border border-emerald-300 flex items-center gap-1 shadow-2xs">
                <UserCheck className="w-3.5 h-3.5 text-[#5F7161]" /> 내가 쓴 글
              </span>
            )}
        ...
        <div className="p-4 sm:p-6 overflow-y-auto overflow-x-hidden space-y-6 flex-1 max-w-full">
          ...
          {(isSecretSuggestion(activeSuggestion) || isSecretSuggestion(suggestion)) && !isUnlocked ? (
            ...
            <>
              ...
              <div>
                {isAdmin && (
                  <div className="mb-2.5 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-amber-50 border border-amber-200 text-amber-900 text-xs font-bold shadow-2xs">
                ...
                <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 leading-snug mb-3">
                  {isAdmin
                    ...
                    : (maskProfanity(stripMetadataMarkers(activeSuggestion.title)) || '제목 없음')}
                </h2>
                ...
                <div className="flex items-center justify-between text-xs text-slate-500 pb-4 border-b border-slate-100">
                  <div className="flex items-center space-x-2">
                    <span className="font-bold text-slate-800 bg-slate-100 px-2.5 py-1 rounded-md">
                      {isAdmin
                        ? (activeSuggestion.authorNickname || '익명의 삼진인')
                        : maskProfanity(activeSuggestion.authorNickname || '익명의 삼진인')}
                    </span>
                    <span>•</span>
              ...
              <div className="text-slate-800 text-sm sm:text-base leading-relaxed whitespace-pre-wrap bg-slate-50/60 p-5 rounded-2xl border border-slate-200/70">
                {isAdmin
                  ...
                  : maskProfanity(stripMetadataMarkers(activeSuggestion.content))}
              </div>
                ...
                <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border-2 border-emerald-300/80 rounded-2xl p-5 shadow-xs space-y-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <h4 className="font-bold text-emerald-950 text-sm">
                          [{activeSuggestion.officialResponse.department}] {isAdmin ? activeSuggestion.officialResponse.authorName : maskProfanity(activeSuggestion.officialResponse.authorName)}
                        </h4>
                  ...
                  <p className="text-emerald-900 text-sm leading-relaxed pt-2 border-t border-emerald-200/60 whitespace-pre-wrap">
                    {isAdmin
                      ...
                      : maskProfanity(activeSuggestion.officialResponse.content)}
                  </p>
                </div>
              )}
              ...
              {isAdmin && (
                <div className="bg-slate-900 text-white rounded-2xl p-5 space-y-4 border border-slate-800">
                  ...
                  {/* Role / Author Selector with Quick Presets */}
                  <div className="space-y-2.5 bg-slate-800/60 p-3.5 rounded-xl border border-slate-700/60">
                    <div className="flex items-center justify-between flex-wrap gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-emerald-300 bg-emerald-950/80 border border-emerald-700/60 px-2 py-0.5 rounded-md">
                          소속: [제53대 학생회]
                        </span>
                      </div>
                      <span className="text-[11px] text-amber-300 font-medium">
                        미리보기: [제53대 학생회] {responseAuthor.trim() || '학생자치부장'}
                      </span>
                    </div>

                    {/* Author Presets Pills */}
                    <div className="flex flex-wrap gap-1.5 pt-0.5">
                      {AUTHOR_ROLE_PRESETS.map((preset) => (
                        <button
                          key={preset}
                          type="button"
                          onClick={() => setResponseAuthor(preset)}
                          className={`text-xs px-3 py-1.5 rounded-lg font-bold transition-all border ${
                            responseAuthor === preset
                    ...
                    <div className="pt-1">
                      <span className="text-[10px] text-slate-400 block mb-1">직책 직접 수정 (필요 시)</span>
                      <input
                        type="text"
                        value={responseAuthor}
                        onChange={(e) => setResponseAuthor(e.target.value)}
                        placeholder="작성자 직책 (예: 학생자치부장, 학생인권부장, 학생회장)"
                        className="w-full bg-slate-900 border border-slate-700 text-xs text-white p-2.5 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 placeholder-slate-500"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-300">공식 답변 내용</label>
                    <textarea
                      rows={3}
                      value={responseContent}
                      onChange={(e) => setResponseContent(e.target.value)}
                      placeholder="학우들에게 전달할 공식 답변 내용을 입력하세요..."
                      className="w-full bg-slate-800 border border-slate-700 text-sm text-white p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-500"
                    />
                  </div>
