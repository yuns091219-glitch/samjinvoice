...
export const SuggestionFormModal: React.FC<SuggestionFormModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const rawSubmittedTags = uniqueFinalTags.length > 0 ? uniqueFinalTags : ['#마산삼진고', '#학생건의'];

    const submittedTitle = title.trim();
    const submittedContent = content.trim();
    const submittedNickname = authorNickname.trim() || '익명의 삼진인';
    ...
    try {
      ...
      netlifyFormData.append('authorNickname', submittedNickname);
      netlifyFormData.append('title', submittedTitle);
      netlifyFormData.append('content', submittedContent);
      netlifyFormData.append('tags', rawSubmittedTags.join(','));
    ...
    onSubmit({
      ...
      authorNickname: submittedNickname,
      isSecret,
      secretPin: isSecret ? secretPin.trim() : undefined,
      tags: rawSubmittedTags,
    });
                ...
                return (
                  ...
                  >
                    <span className="inline-flex items-center gap-2">
                      <CatIcon className={`w-4 h-4 shrink-0 ${isSelected ? 'text-white' : 'text-[#5F7161]'}`} />
