...
function formatSafeSuggestion(item: Suggestion, isAdminUser: boolean = false, keepUnmaskedIfVerified: boolean = false): Suggestion {
  ...
  // For regular (non-admin) users, apply profanity masking filter
  if (!isAdminUser) {
    const maskedComments = (safeItem.comments || []).map((c) => ({
      ...c,
      authorNickname: maskProfanity(c.authorNickname || '익명의 삼진인'),
      content: maskProfanity(c.content || ''),
    }));

    if (isItemSecret && !keepUnmaskedIfVerified) {
      return {
        ...safeItem,
        id: stringId,
        category: effectiveCategory,
        authorNickname: maskProfanity(effectiveAuthor),
        title: maskProfanity(safeItem.title || ''),
        isSecret: true,
        tags: itemTags.map((t) => maskProfanity(t)),
    ...
    return {
      ...
      authorNickname: maskProfanity(effectiveAuthor),
      title: maskProfanity(safeItem.title || ''),
      content: maskProfanity(safeItem.content || ''),
  ...
  // Admin user: Return 100% unmasked raw original data
  const realContent = originalContentStore.get(stringId) || item.content;
  const unmaskedContent = (realContent && !realContent.startsWith('🔒 비밀글입니다'))
    ? realContent
    : (safeItem.content || '');
    ...
    try {
      ...
      if (found.isSecret && !isAdminUser && !isPinCorrect) {
        res.json({
          ...
          title: maskProfanity(found.title || ''),
          authorNickname: maskProfanity(found.authorNickname || '익명의 삼진인'),
          tags: (found.tags || []).map((t) => maskProfanity(t)),
          secretPin: undefined,
          isLocked: true,
        });
        return;
      ...
      const finalContent = (realContent && !realContent.startsWith('🔒 비밀글입니다')) ? realContent : found.content;

      if (!isAdminUser) {
        res.json({
          ...safeFound,
          title: maskProfanity(safeFound.title || ''),
          authorNickname: maskProfanity(safeFound.authorNickname || '익명의 삼진인'),
          content: maskProfanity(finalContent || ''),
          tags: (safeFound.tags || []).map((t) => maskProfanity(t)),
          comments: comments.map((c) => ({
            ...c,
            authorNickname: maskProfanity(c.authorNickname || '익명의 삼진인'),
            content: maskProfanity(c.content || ''),
  ...
  app.post('/api/suggestions', async (req, res) => {
    ...
    const rawAuthor = authorNickname ? String(authorNickname).trim() : '익명의 삼진인';
    const rawTags = (Array.isArray(tags) ? tags : ['#마산삼진고', '#건의사항']).map((t: string) => String(t));
    ...
    try {
      ...
      const cleanTags = rawTags.length > 0 ? rawTags : (newSuggestion.tags && newSuggestion.tags.length > 0 ? newSuggestion.tags : ['#마산삼진고', '#건의사항']);
  ...
  app.post('/api/suggestions/:id/comments', async (req, res) => {
    ...
    const rawAuthor = authorNickname ? authorNickname.trim() : '익명의 삼진인';
    const rawCommentContent = content.trim();
  ...
  app.patch('/api/suggestions/:id/status', async (req, res) => {
    ...
    try {
      const updated = await updateStatusInSupabase(
        ...
        officialResponse?.authorName,
        officialResponse?.department
      );
      ...
      const safeUpdated = formatSafeSuggestion(updated, true);
